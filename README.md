...
Loads `results/model.keras` + `model_meta.json` and opens your webcam.
**SPACE** to start a sign, **SPACE** again to stop and classify, **R** to
clear, **Q**/**Esc** to quit. It's segment-based (record a whole sign, then
predict) — the model expects one complete sign per input and has no "idle"
class, so continuous always-on prediction would be unreliable. A confidence
